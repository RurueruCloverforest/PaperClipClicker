...
export function machineUnitProduction(state: GameState, id: MachineId): number {
  ...
  return getMachine(id).baseProduction * multiplier * milestoneStatus(state.machines[id]).multiplier * achievementProductionMultiplier(state) * signalEquipmentMultiplier(state) * rebootMultiplier(state) * overclockMultiplier(state, id);
}
