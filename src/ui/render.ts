...
import { FOLD_COOLDOWN_MS, canStartFold, foldCost, isFoldCooling, isFoldUnlocked } from '../game/fold';
import { GAUGE_LABELS, isGaugeReady, isGaugeUnlocked, type GaugeId } from '../game/gauge';
...
import { PULSE_COOLDOWN_MS, isPulseReady, isPulseUnlocked, pulseReward } from '../game/pulse';
import { isScrapUnlocked, scrapCapacity } from '../game/scrap';
import { TRIM_BONUS, TRIM_CAP, canBuyTrim, isTrimMaxed, isTrimUnlocked, trimCost } from '../game/trim';
...
export class GameUi {
  ...
  private readonly capacitor: HTMLElement;
  private readonly capacitorValue: HTMLElement;
  ...
  private readonly foldStatus: HTMLElement;
  private readonly foldPrice: HTMLElement;
  private readonly foldProgress: HTMLElement;
  private readonly foldButton: HTMLButtonElement;
  private readonly foldCount: HTMLElement;
  private readonly gauge: HTMLElement;
  private readonly gaugeStatus: HTMLElement;
  private readonly gaugeButtons: HTMLButtonElement[];
  private readonly gaugeHits: HTMLElement;
  private readonly catalyst: HTMLElement;
  private readonly catalystStatus: HTMLElement;
  private readonly catalystDrain: HTMLElement;
  private readonly catalystButton: HTMLButtonElement;
  private readonly catalystSeconds: HTMLElement;
  private readonly dispatch: HTMLElement;
  private readonly dispatchStatus: HTMLElement;
  private readonly dispatchProgress: HTMLElement;
  private readonly dispatchButton: HTMLButtonElement;
  private readonly dispatchClaims: HTMLElement;
  private readonly patch: HTMLElement;
  private readonly patchStatus: HTMLElement;
  private readonly patchPrice: HTMLElement;
  ...
  private readonly pulseStatus: HTMLElement;
  private readonly pulseProgress: HTMLElement;
  private readonly pulseButton: HTMLButtonElement;
  private readonly pulseCount: HTMLElement;
  private readonly scrap: HTMLElement;
  private readonly scrapValue: HTMLElement;
  private readonly scrapProgress: HTMLElement;
  private readonly scrapButton: HTMLButtonElement;
  private readonly scrapClaims: HTMLElement;
  private readonly trim: HTMLElement;
  private readonly trimStatus: HTMLElement;
  private readonly trimPrice: HTMLElement;
  private readonly trimProgress: HTMLElement;
  private readonly trimButton: HTMLButtonElement;
  private readonly trimCount: HTMLElement;
  ...
  constructor(private readonly root: HTMLElement, private readonly actions: UiActions) {
    ...
    this.capacitor = required(root, '#capacitor');
    this.capacitorValue = required(root, '#capacitor-value');
    ...
    this.fold = required(root, '#fold');
    this.foldStatus = required(root, '#fold-status');
    this.foldPrice = required(root, '#fold-price');
    this.foldProgress = required(root, '#fold-progress');
    this.foldButton = required(root, '#start-fold');
    this.foldCount = required(root, '#fold-count');
    this.gauge = required(root, '#gauge');
    this.gaugeStatus = required(root, '#gauge-status');
    this.gaugeButtons = [...root.querySelectorAll<HTMLButtonElement>('[data-gauge]')];
    this.gaugeHits = required(root, '#gauge-hits');
    this.catalyst = required(root, '#catalyst');
    this.catalystStatus = required(root, '#catalyst-status');
    this.catalystDrain = required(root, '#catalyst-drain');
    this.catalystButton = required(root, '#toggle-catalyst');
    this.catalystSeconds = required(root, '#catalyst-seconds');
    this.dispatch = required(root, '#dispatch');
    this.dispatchStatus = required(root, '#dispatch-status');
    this.dispatchProgress = required(root, '#dispatch-progress');
    this.dispatchButton = required(root, '#dispatch-action');
    this.dispatchClaims = required(root, '#dispatch-claims');
    this.patch = required(root, '#patch');
    this.patchStatus = required(root, '#patch-status');
    this.patchPrice = required(root, '#patch-price');
    ...
    this.pulse = required(root, '#pulse');
    this.pulseStatus = required(root, '#pulse-status');
    this.pulseProgress = required(root, '#pulse-progress');
    this.pulseButton = required(root, '#claim-pulse');
    this.pulseCount = required(root, '#pulse-count');
    this.scrap = required(root, '#scrap');
    this.scrapValue = required(root, '#scrap-value');
    this.scrapProgress = required(root, '#scrap-progress');
    this.scrapButton = required(root, '#claim-scrap');
    this.scrapClaims = required(root, '#scrap-claims');
    this.trim = required(root, '#trim');
    this.trimStatus = required(root, '#trim-status');
    this.trimPrice = required(root, '#trim-price');
    this.trimProgress = required(root, '#trim-progress');
    this.trimButton = required(root, '#buy-trim');
    this.trimCount = required(root, '#trim-count');
  ...
  render(state: GameState, force = false): void {
    ...
    this.foldCount.textContent = String(state.foldCount);
    this.gaugeHits.textContent = String(state.gaugeHits);
    ...
    this.pulseCount.textContent = String(state.pulseCount);
    this.scrapClaims.textContent = String(state.scrapClaims);
    this.trimCount.textContent = String(state.trimCount);
    ...
    this.renderPulse(state, format);
    this.renderScrap(state, format);
    this.renderTrim(state, format);
  ...
  private renderCapacitor(state: GameState, format: (value: number) => string): void {
    const unlocked = isCapacitorUnlocked(state);
    this.capacitor.hidden = !unlocked;
    if (!unlocked) return;
    const capacity = capacitorCapacity(state);
    const stored = state.capacitorStored;
    const ratio = capacity > 0 ? Math.min(1, stored / capacity) : 0;
    const full = stored + Number.EPSILON >= capacity && capacity > 0;
    this.capacitor.dataset.full = full ? 'true' : 'false';
    this.capacitorValue.textContent = `${format(stored)} / ${format(capacity)}${full ? ' FULL' : ''}`;
    this.capacitorProgress.style.width = `${ratio * 100}%`;
    this.capacitorProgress.closest('[role="progressbar"]')?.setAttribute('aria-valuenow', String(Math.floor(ratio * 100)));
    this.capacitorButton.disabled = stored <= 0;
    this.capacitorButton.setAttribute('aria-label', stored > 0 ? `余剰コンデンサから${format(stored)}クリップを回収` : '余剰コンデンサは空です');
  }

  private renderFold(state: GameState, format: (value: number) => string): void {
    const unlocked = isFoldUnlocked(state);
    this.fold.hidden = !unlocked;
    if (!unlocked) return;
    const now = Date.now();
    const cooling = isFoldCooling(state, now);
    const rate = productionPerSecond(state);
    const cost = foldCost(rate);
    const remainMs = Math.max(0, state.foldExpiresAt - now);
    const remainRatio = cooling ? remainMs / FOLD_COOLDOWN_MS : 1;
    const status = cooling ? `COOL ${this.formatCountdown(remainMs / 1000)}` : 'READY';
    this.fold.dataset.ready = cooling ? 'false' : 'true';
    this.foldStatus.textContent = status;
    this.foldPrice.textContent = `${format(cost)} clips`;
    this.foldProgress.style.width = `${remainRatio * 100}%`;
    this.foldProgress.closest('[role="progressbar"]')?.setAttribute('aria-valuenow', String(Math.floor(remainRatio * 100)));
    const canStart = canStartFold(state, rate, now);
    this.foldButton.disabled = !canStart;
    this.foldButton.setAttribute('aria-label', cooling
      ? `折畳圧縮、冷却残り${this.formatCountdown(remainMs / 1000)}`
      : canStart
        ? `${format(cost)}クリップで折畳圧縮する`
        : '折畳圧縮の費用が足りません');
  }

  private renderGauge(state: GameState): void {
    const unlocked = isGaugeUnlocked(state);
    this.gauge.hidden = !unlocked;
    if (!unlocked) return;
    const now = Date.now();
    const ready = isGaugeReady(state, now);
    const remainMs = Math.max(0, state.gaugeReadyAt - now);
    const targetLabel = GAUGE_LABELS[state.gaugeTarget];
    this.gauge.dataset.ready = ready ? 'true' : 'false';
    this.gaugeStatus.textContent = ready ? `TARGET ${targetLabel}` : `COOL ${this.formatCountdown(remainMs / 1000)}`;
    for (const button of this.gaugeButtons) {
      const id = Number(button.dataset.gauge) as GaugeId;
      const label = GAUGE_LABELS[id];
      button.disabled = !ready;
      button.setAttribute('aria-label', ready
        ? `線径${label}を選ぶ、目標${targetLabel}`
        : `線径選別、冷却残り${this.formatCountdown(remainMs / 1000)}`);
    }
  }

  private renderCatalyst(state: GameState, format: (value: number) => string): void {
    const unlocked = isCatalystUnlocked(state);
    this.catalyst.hidden = !unlocked;
    if (!unlocked) return;
    const unboosted = productionPerSecond(state) / catalystMultiplier(state);
    const drain = catalystDrainPerSecond(unboosted);
    const active = state.catalystActive;
    this.catalyst.dataset.active = active ? 'true' : 'false';
    this.catalystStatus.textContent = active ? 'ACTIVE' : 'IDLE';
    this.catalystDrain.textContent = `${format(drain)} / 秒`;
    this.catalystButton.setAttribute('aria-pressed', active ? 'true' : 'false');
    this.catalystButton.disabled = !active && !canEnableCatalyst(state, unboosted);
    this.catalystButton.setAttribute('aria-label', active
      ? `触媒滴下を停止、消費${format(drain)}クリップ毎秒`
      : `触媒滴下を開始、消費${format(drain)}クリップ毎秒`);
  }

  private renderDispatch(state: GameState, format: (value: number) => string): void {
    const unlocked = isDispatchUnlocked(state);
    this.dispatch.hidden = !unlocked;
    if (!unlocked) return;
    const now = Date.now();
    const status = dispatchStatus(state, now);
    const cap = dispatchCapacity(productionPerSecond(state));
    const stored = state.dispatchStored;
    const remainMs = Math.max(0, state.dispatchReturnsAt - now);
    const fillingRatio = cap > 0 ? Math.min(1, stored / cap) : 0;
    const ratio = status === 'transit' ? remainMs / DISPATCH_TRANSIT_MS : status === 'ready' ? 1 : fillingRatio;
    const statusText = status === 'transit'
      ? `TRANSIT ${this.formatCountdown(remainMs / 1000)}`
      : status === 'ready'
        ? `READY ${format(stored)}`
        : `FILLING ${format(stored)} / ${format(cap)}`;
    this.dispatch.dataset.ready = status === 'ready' ? 'true' : 'false';
    this.dispatchStatus.textContent = statusText;
    this.dispatchProgress.style.width = `${ratio * 100}%`;
    this.dispatchProgress.closest('[role="progressbar"]')?.setAttribute('aria-valuenow', String(Math.floor(ratio * 100)));
    this.dispatchButton.textContent = status === 'ready' ? '回収' : status === 'transit' ? '輸送' : '出荷';
    this.dispatchButton.disabled = status === 'transit' || (status === 'filling' && stored <= 0);
    this.dispatchButton.setAttribute('aria-label', status === 'ready'
      ? `出荷便から${format(stored * DISPATCH_REWARD_MULT)}クリップを回収`
      : status === 'transit'
        ? `出荷便、輸送残り${this.formatCountdown(remainMs / 1000)}`
        : stored > 0
          ? `${format(stored)}クリップを出荷する`
          : '出荷便は空です');
  }

  private renderPatch(state: GameState, format: (value: number) => string): void {
    const unlocked = isPatchUnlocked(state);
    this.patch.hidden = !unlocked;
    if (!unlocked) return;
    const maxed = isPatchMaxed(state);
    const bonusPercent = Math.round((patchMultiplier(state) - 1) * 100);
    const rate = productionPerSecond(state);
    const cost = patchCost(state, rate);
    this.patch.dataset.max = maxed ? 'true' : 'false';
    this.patchStatus.textContent = maxed ? `REV ${state.patchCount} +${bonusPercent}% MAX` : `REV ${state.patchCount} +${bonusPercent}%`;
    this.patchPrice.textContent = maxed ? 'MAX' : `${format(cost)} clips`;
    this.patchProgress.style.width = `${(state.patchCount / PATCH_CAP) * 100}%`;
    this.patchProgress.closest('[role="progressbar"]')?.setAttribute('aria-valuenow', String(state.patchCount));
    const canBuy = canBuyPatch(state, rate);
    this.patchButton.disabled = !canBuy;
    this.patchButton.setAttribute('aria-label', maxed
      ? '最適化改訂は上限です'
      : canBuy
        ? `${format(cost)}クリップで最適化改訂する`
        : '最適化改訂の費用が足りません');
  }
  ...
  private renderPulse(state: GameState, format: (value: number) => string): void {
    const unlocked = isPulseUnlocked(state);
    this.pulse.hidden = !unlocked;
    if (!unlocked) return;
    const now = Date.now();
    const ready = isPulseReady(state, now);
    const remainMs = Math.max(0, state.pulseReadyAt - now);
    const ratio = ready ? 1 : 1 - remainMs / PULSE_COOLDOWN_MS;
    const reward = pulseReward(productionPerSecond(state));
    this.pulse.dataset.ready = ready ? 'true' : 'false';
    this.pulseStatus.textContent = ready ? 'READY' : `CHARGE ${this.formatCountdown(remainMs / 1000)}`;
    this.pulseProgress.style.width = `${Math.min(1, Math.max(0, ratio)) * 100}%`;
    this.pulseProgress.closest('[role="progressbar"]')?.setAttribute('aria-valuenow', String(Math.floor(Math.min(1, Math.max(0, ratio)) * 100)));
    this.pulseButton.disabled = !ready;
    this.pulseButton.setAttribute('aria-label', ready
      ? `同期パルスを回収、${format(reward)}クリップ`
      : `同期パルス、充電残り${this.formatCountdown(remainMs / 1000)}`);
  }

  private renderScrap(state: GameState, format: (value: number) => string): void {
    const unlocked = isScrapUnlocked(state);
    this.scrap.hidden = !unlocked;
    if (!unlocked) return;
    const capacity = scrapCapacity(productionPerSecond(state));
    const stored = state.scrapStored;
    const ratio = capacity > 0 ? Math.min(1, stored / capacity) : 0;
    const full = stored + Number.EPSILON >= capacity && capacity > 0;
    this.scrap.dataset.full = full ? 'true' : 'false';
    this.scrapValue.textContent = `${format(stored)} / ${format(capacity)}${full ? ' FULL' : ''}`;
    this.scrapProgress.style.width = `${ratio * 100}%`;
    this.scrapProgress.closest('[role="progressbar"]')?.setAttribute('aria-valuenow', String(Math.floor(ratio * 100)));
    this.scrapButton.disabled = stored <= 0;
    this.scrapButton.setAttribute('aria-label', stored > 0 ? `端材から${format(stored)}クリップを回収` : '端材箱は空です');
  }

  private renderTrim(state: GameState, format: (value: number) => string): void {
    const unlocked = isTrimUnlocked(state);
    this.trim.hidden = !unlocked;
    if (!unlocked) return;
    const maxed = isTrimMaxed(state);
    const discountPercent = Math.round(Math.min(TRIM_CAP, state.trimCount) * TRIM_BONUS * 100);
    const rate = productionPerSecond(state);
    const cost = trimCost(state, rate);
    this.trim.dataset.max = maxed ? 'true' : 'false';
    this.trimStatus.textContent = maxed ? `TRIM ${state.trimCount} -${discountPercent}% MAX` : `TRIM ${state.trimCount} -${discountPercent}%`;
    this.trimPrice.textContent = maxed ? 'MAX' : `${format(cost)} clips`;
    this.trimProgress.style.width = `${(state.trimCount / TRIM_CAP) * 100}%`;
    this.trimProgress.closest('[role="progressbar"]')?.setAttribute('aria-valuenow', String(state.trimCount));
    const canBuy = canBuyTrim(state, rate);
    this.trimButton.disabled = !canBuy;
    this.trimButton.setAttribute('aria-label', maxed
      ? '調達削りは上限です'
      : canBuy
        ? `${format(cost)}クリップで調達削りする`
        : '調達削りの費用が足りません');
  }
  ...
  private bindEvents(): void {
    ...
    this.capacitorButton.addEventListener('click', this.actions.claimCapacitor);
    this.foldButton.addEventListener('click', this.actions.startFold);
    for (const button of this.gaugeButtons) {
      button.addEventListener('click', () => this.actions.pickGauge(Number(button.dataset.gauge) as GaugeId));
    }
    ...
    this.pulseButton.addEventListener('click', this.actions.claimPulse);
    this.scrapButton.addEventListener('click', this.actions.claimScrap);
    this.trimButton.addEventListener('click', this.actions.buyTrim);
